...
const ProviderAvatar = ({ initials }) => (
  <div className="w-12 h-12 sm:w-14 sm:h-14 flex items-center justify-center rounded-full bg-gray-100 border border-gray-200 shadow-inner overflow-hidden flex-shrink-0">
    <img
      ...
      onError={(e) => {
        e.currentTarget.src = "https://placehold.co/100x100/A0BFFF/3D28E8/png?text=D";
      }}
    />
  </div>
);

export default function AssessmentById() {
    ...
    <div className="flex flex-col min-h-screen pt-16 text-black">
      ...
      <main className="flex-1 bg-[rgb(246,244,255)] px-4 sm:px-6 lg:px-10 py-8 sm:py-12">
            ...
            <div className="flex-1">
              <div className="flex flex-col sm:flex-row justify-between items-start gap-4 mb-6">
                <div>
                  ...
                  <h2 className="text-2xl sm:text-3xl font-bold">{conditionName}</h2>
                  {severity && (
                    <span className={`inline-block mt-2 px-3 py-1 rounded-full text-xs font-semibold ${severityColor[severity]}`}>
              ...
              <p className="text-gray-700 mb-6 leading-relaxed text-sm sm:text-base">{conditionDesc}</p>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-6 pt-4 sm:pt-6 border-t border-gray-100">
                <div>
                  ...
                  <p className="text-gray-600 text-sm">{triggers?.join(", ")}</p>
                </div>
                <div>
                  <h3 className="font-semibold mb-1 text-gray-800 text-sm sm:text-base">Initial Self-Care</h3>
                  <p className="text-gray-600 text-sm">{initialSelfCare?.join(", ")}</p>
                </div>
              </div>
              ...
              {disclaimer && (
                <div className="mt-6 p-3 rounded-xl text-xs sm:text-sm bg-[rgb(231,225,253)]">
                  <p className="text-gray-600">
                    <span className="font-semibold mr-1 text-[rgb(61,40,223)]">Disclaimer:</span>
                    {disclaimer}
                  </p>
                </div>
              )}
        ...
        <section className="max-w-6xl mx-auto mb-10 sm:mb-12">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 sm:gap-6">
            {(nextSteps ?? []).map((item, i) => {
              const icon = iconMap[item.icon] || <Info className="w-6 h-6 text-[rgb(61,40,223)]" />;
              return (
                <div key={i} onClick={() => item.url && window.open(item.url, "_blank")} className="bg-white p-5 sm:p-6 rounded-2xl shadow-lg border border-gray-100 flex flex-col items-start hover:shadow-xl transition cursor-pointer">
                  <div className="mb-4 p-3 rounded-full bg-[rgb(61,40,223)/.1]">{icon}</div>
          ...
          <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-8 gap-4 border-b border-gray-200">
            <div className="flex flex-wrap gap-8 text-sm sm:text-base font-medium">
              {["Overview", "Providers", "Products"].map((tab) => {
                ...
                return (
                  <button key={tab} onClick={() => setActiveTab(tab)} className={`relative pb-3 transition-all duration-200 ${isActive ? "text-[rgb(61,40,223)] font-semibold" : "text-gray-500 hover:text-gray-700"}`}>
                    {tab}
                    {isActive && <span className="absolute bottom-0 left-0 right-0 h-0.5 bg-[rgb(61,40,223)] rounded-full" />}
                  </button>
                );
              })}
            </div>
            ...
            <section className="mt-8">
             {(warnings || []).length > 0 ? (
                <div className="space-y-3">
                  {warnings.map((w, i) => (
                    <div key={i} className="bg-red-50 border border-red-200 text-red-800 p-5 rounded-2xl flex items-start gap-3 shadow-sm">
                      <AlertCircle className="w-5 h-5 text-red-600 mt-0.5 flex-shrink-0" />
                      <div className="flex-1">
                        <p className="font-semibold text-red-900">{w.message}</p>
                        {w.action && <p className="text-sm mt-1.5 font-medium text-red-700">Action: {w.action}</p>}
                      </div>
                    </div>
                  ))}
                </div>
                ...
                <div className="bg-gradient-to-r from-blue-50 to-indigo-50 border border-blue-200 rounded-2xl p-8 text-center">
                  ...
                  <p className="text-lg font-medium text-gray-800">All systems operational</p>
                  <p className="text-sm text-gray-600 mt-1">No warnings or alerts at this time.</p>
                </div>
              )}
            </section>
          )}
            ...
            <section className="max-w-6xl mx-auto mb-12 sm:mb-16">
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                ...
                <div className="flex flex-col gap-4">
                  {(providers || []).map((doc) => (
                    <div key={doc.placeId || doc.name} className="bg-white p-5 rounded-2xl shadow-lg border border-gray-100 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 hover:shadow-xl transition-shadow duration-300">
                      <div className="flex items-center gap-4 w-full sm:w-auto">
                        <ProviderAvatar initials={doc.initials || getInitials(doc.name)} />
                        <div className="flex-1">
                          <h4 className="font-semibold text-gray-800">{doc.name}</h4>
                          <p className="text-sm text-gray-600">{doc.specialty} · {doc.distance}</p>
                      ...
                      <Link href={doc.bookingUrl} target="_blank" rel="noopener noreferrer" className="w-full sm:w-auto bg-[rgb(61,40,223)] hover:bg-[rgb(103,18,232)] text-white px-5 py-2.5 rounded-full text-sm font-semibold transition-colors">
                        Book Appointment
                      </Link>
                    </div>
                  ))}
            ...
            <section className="mt-8">
              {(products|| []).length > 0 ? (
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
                  {products.map((p, i) => (
                    <div key={i} className="group bg-white rounded-2xl shadow-md border border-gray-100 p-6 hover:shadow-xl hover:border-[rgb(61,40,223)] transition-all duration-300 cursor-pointer">
                      ...
                      <h4 className="font-bold text-xl text-gray-900">{p.name}</h4>
                      <p className="text-sm font-medium text-[rgb(61,40,223)] mt-1">{p.type}</p>
                      <p className="text-sm text-gray-600 mt-3 leading-relaxed">{p.description}</p>
                      <div className="flex gap-2 mt-3">
                        {p.isPrescription ? (
                          <span className="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-red-100 text-red-800">Prescription Required</span>
                        ) : (
                          <span className="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-green-100 text-green-800">Over the Counter</span>
                        )}
                        {p.requiresConsultation && (
                          <span className="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-amber-100 text-amber-800">Consultation Suggested</span>
                        )}
                      </div>
                      <p className="text-xs text-gray-500 mt-4 italic">{p.disclaimer}</p>
                      <div className="mt-5 pt-4 border-t border-gray-100">
                        <Link href={p.purchaseUrl} target="_blank" rel="noopener noreferrer" className="inline-flex items-center text-sm font-medium text-[rgb(61,40,223)] group-hover:underline">
                          Buy
        ...
        <section className="max-w-6xl mx-auto p-5 sm:p-6 flex flex-col sm:flex-row justify-between items-center gap-4 rounded-2xl shadow-lg mb-10 bg-[rgb(231,225,253)]">
          <p className="text-gray-800 font-medium text-center sm:text-left text-base sm:text-lg">Want to explore other possibilities?</p>
          <div className="flex flex-col sm:flex-row gap-3 w-full sm:w-auto">
            <Link href="/chat" className="w-full sm:w-auto border px-5 py-2.5 rounded-full font-semibold transition text-sm bg-white border-[rgb(61,40,223)] text-[rgb(61,40,223)] hover:bg-[rgb(61,40,223)/.05]">
              Refine Your Results
            </Link>
            <Link className="w-full sm:w-auto text-white px-5 py-2.5 rounded-full font-semibold transition flex items-center justify-center gap-2 text-sm bg-[rgb(61,40,223)] hover:bg-[rgb(103,18,232)]" href="/">
